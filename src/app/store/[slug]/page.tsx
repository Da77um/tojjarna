...
// ─── Section renderers ────────────────────────────────────────────────────────
function SectionRenderer({ section, primary, radius, products, slug, t, dir }: {
    section: ThemeSection; primary: string; radius: number; products: any[]; slug: string; t: any; dir: string
}) {
    ...
    switch (section.type) {
            ...
            return (
                <div style={{
                    ...
                    <h1 style={{ color: s.text_color || '#fff', fontSize: 'clamp(24px,5vw,42px)', fontWeight: 900, marginBottom: 14, lineHeight: 1.3, textShadow: '0 2px 12px rgba(0,0,0,0.25)' }}>
                        {dir === 'rtl' ? (s.title_ar || t.storefront.welcome) : (s.title_en || s.title_ar || t.storefront.welcome)}
                    </h1>
                    {(dir === 'rtl' ? s.subtitle_ar : s.subtitle_en || s.subtitle_ar) && <p style={{ color: `${s.text_color || '#fff'}cc`, fontSize: 17, marginBottom: 28, maxWidth: 560 }}>{dir === 'rtl' ? s.subtitle_ar : s.subtitle_en || s.subtitle_ar}</p>}
                    {(dir === 'rtl' ? s.cta_text_ar : s.cta_text_en || s.cta_text_ar) && (
                        <Link href={s.cta_url || `/store/${slug}/products`}
                            ...
                            {dir === 'rtl' ? s.cta_text_ar : s.cta_text_en || s.cta_text_ar} {dir === 'rtl' ? <ChevronLeft size={16} /> : <ChevronRight size={16} />}
                        </Link>
                    )}
                </div>
            )
        ...
        case 'featured_products': {
            ...
            const title = dir === 'rtl' ? s.title_ar : s.title_en || s.title_ar
            return (
                <div style={{ padding: '56px 24px', maxWidth: 1200, margin: '0 auto' }}>
                    {title && (
                        <div style={{ textAlign: 'center', marginBottom: 36 }}>
                            <h2 style={{ fontSize: 26, fontWeight: 900, color: '#111827' }}>{title}</h2>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(cols, 4)}, 1fr)`, gap: 20 }}>
                        {(display.length ? display : Array.from({ length: 4 }, (_, i) => null)).map((product, i) => (
                            product ? (
                                <ProductCard key={product.id} product={product} primary={primary} radius={radius} slug={slug} t={t} dir={dir} />
                            ) : (
            ...
            return (
                <div style={{ padding: '48px 0' }}>
                    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <h2 style={{ fontSize: 22, fontWeight: 900, color: '#111827' }}>{title || t.storefront.newArrivals}</h2>
                        {s.show_all_link && <Link href={`/store/${slug}/products`} style={{ color: primary, fontSize: 14, fontWeight: 700, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>{t.storefront.viewAll} {dir === 'rtl' ? <ChevronLeft size={14} /> : <ChevronRight size={14} />}</Link>}
                    </div>
                        ...
                        {(display.length ? display : Array.from({ length: 6 }, () => null)).map((product, i) =>
                            product ? (
                                <div key={product.id} style={{ minWidth: 200, flexShrink: 0 }}>
                                    <ProductCard product={product} primary={primary} radius={radius} slug={slug} compact t={t} dir={dir} />
                                </div>
                            ) : (
        ...
        case 'countdown_timer':
            return <CountdownSection s={s} primary={primary} radius={radius} t={t} dir={dir} />
                    ...
                    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
                            {(s.items?.length ? s.items : [
                                { name: 'Ahmad', text: 'Great products and fast shipping!', stars: 5 },
                                { name: 'Fatima', text: 'Excellent customer service.', stars: 5 },
                                { name: 'Khalid', text: 'Competitive prices and high quality.', stars: 4 },
                            ]).map((t: any, i: number) => (
        ...
        case 'rich_text': {
            const content = dir === 'rtl' ? s.content_ar : s.content_en || s.content_ar
            return (
            ...
            return (
                <div style={{ padding: '56px 24px', maxWidth: 1200, margin: '0 auto' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${s.columns || 3}, 1fr)`, gap: 20 }}>
                        {(s.items?.length ? s.items : ['Category 1', 'Category 2', 'Category 3'].slice(0, s.columns || 3)).map((item: any, i: number) => (
                            <div key={i} style={{
        ...
        case 'announcement_bar':
            return (
                <div style={{ background: s.background_color || primary, color: s.text_color || 'white', textAlign: 'center', padding: '11px 16px', fontSize: 14, fontWeight: 600 }}>
                    {dir === 'rtl' ? s.text_ar : s.text_en || s.text_ar || ''}
                </div>
            )
...
// ─── Product Card ─────────────────────────────────────────────────────────────
function ProductCard({ product, primary, radius, slug, compact, t, dir }: any) {
        ...
        <Link href={`/store/${slug}/products/${product.id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
            ...
            >
                ...
                <div style={{ height: compact ? 140 : 220, overflow: 'hidden', position: 'relative' }}>
                    ...
                    {product.is_featured && (
                        <div style={{
                            position: 'absolute', top: 12, [dir === 'rtl' ? 'right' : 'left']: 12,
                            background: '#C6A75E', color: '#111111',
                            borderRadius: 6, padding: '4px 10px',
                            fontSize: 11, fontWeight: 900, letterSpacing: '0.04em',
                        }}>{t.storefront.exclusive}</div>
                    )}
                    {/* Discount badge */}
                    {product.compare_price && Number(product.compare_price) > Number(product.price) && (
                        <div style={{
                            position: 'absolute', top: product.is_featured ? 44 : 12, [dir === 'rtl' ? 'right' : 'left']: 12,
                    ...
                    {product.stock === 0 && (
                        <div style={{
                            ...
                            <span style={{ color: 'white', fontWeight: 900, fontSize: 14, background: 'rgba(0,0,0,0.6)', padding: '6px 16px', borderRadius: 8 }}>{t.storefront.outOfStock}</span>
                        </div>
                    )}
                </div>
                ...
                <div style={{ padding: compact ? '12px 14px' : '16px 18px 20px' }}>
                    <div style={{ fontWeight: 700, fontSize: compact ? 13 : 15, marginBottom: 6, color: '#111111', lineHeight: 1.4 }}>{productName}</div>
                    {!compact && (
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                            ...
                            <span style={{ color: '#6B6058', fontSize: 11, [dir === 'rtl' ? 'marginRight' : 'marginLeft']: 3 }}>{t.common.currency}</span>
                            {product.compare_price && (
                                <div style={{ color: '#A09080', fontSize: 11, textDecoration: 'line-through' }}>{Number(product.compare_price).toFixed(3)} {t.common.currency}</div>
                            )}
                        </div>
                        {!compact && (
                            <button
...
// ─── Countdown ────────────────────────────────────────────────────────────────
function CountdownSection({ s, primary, radius, t }: any) {
    ...
    const title = s.title_ar || s.title_en || t.storefront.limitedOffer

    return (
        <div style={{ background: bg, padding: '48px 24px', textAlign: 'center' }}>
            <h2 style={{ color: fg, fontSize: 24, fontWeight: 900, marginBottom: 24 }}>{title}</h2>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 16, direction: 'ltr' }}>
                {[{ v: timeLeft.d, l: t.storefront.days }, { v: timeLeft.h, l: t.storefront.hours }, { v: timeLeft.m, l: t.storefront.minutes }, { v: timeLeft.sec, l: t.storefront.seconds }].map(({ v, l }) => (
                    <div key={l} style={{ textAlign: 'center', background: 'rgba(255,255,255,0.15)', borderRadius: radius, padding: '16px 20px', minWidth: 72, backdropFilter: 'blur(8px)' }}>
                        ...
                        <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)', marginTop: 6, direction: 'rtl' }}>{l}</div>
                    </div>
                ))}
            </div>
        </div>
...
export default function StorefrontPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const [activeCategory, setActiveCategory] = useState(t.storefront.all)
    const [categories, setCategories] = useState<string[]>([])
    const [search, setSearch] = useState('')

    useEffect(() => {
        setActiveCategory(t.storefront.all)
    }, [t])
        ...
        async function fetchStoreData() {
            try {
                ...
                if (productsData) {
                    ...
                    const cats = Array.from(new Set(productsData.map(p => dir === 'rtl' ? p.categories?.name_ar : p.categories?.name_en || p.categories?.name_ar).filter(Boolean))) as string[]
                    setCategories([t.storefront.all, ...cats])
                }
            } catch (err) { console.error(err) } finally { setLoading(false) }
        }
        fetchStoreData()
    }, [supabase, slug, dir, t.storefront.all])
    ...
    if (!store) return (
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#FAFAFA' }}>
            ...
            <h2 style={{ marginBottom: 16 }}>{t.storefront.storeNotFound}</h2>
            <Link href="/" style={{ color: '#6C3CE1', fontWeight: 700 }}>{t.storefront.backToHome}</Link>
        </div>
    )
    ...
    const filteredProducts = products.filter(p => {
        const catName = dir === 'rtl' ? p.categories?.name_ar : p.categories?.name_en || p.categories?.name_ar
        const matchCat = activeCategory === t.storefront.all || catName === activeCategory
        const matchSearch = !search || p.name_ar?.toLowerCase().includes(search.toLowerCase()) || p.name_en?.toLowerCase().includes(search.toLowerCase())
        return matchCat && matchSearch
    })

    return (
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#FAFAFA', fontFamily: `${font}, Tajawal, Inter, sans-serif`, direction: dir as 'rtl' | 'ltr' }}>
                ...
                <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: 64 }}>
                    ...
                    <nav style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        {(themeGlobal?.nav_links?.length
                            ...
                            : [{ label: t.storefront.home, url: '/', type: 'builtin' }, { label: t.storefront.products, url: '/products', type: 'builtin' }]
                        ).map((link: any) => {
                    ...
                    <div style={{ position: 'relative', width: 260, display: 'flex' }}>
                        <Search size={15} style={{ position: 'absolute', [dir === 'rtl' ? 'right' : 'left']: 12, top: '50%', transform: 'translateY(-50%)', color: '#9CA3AF' }} />
                        <input value={search} onChange={e => setSearch(e.target.value)}
                            placeholder={t.storefront.searchPlaceholder} style={{ width: '100%', [dir === 'rtl' ? 'paddingRight' : 'paddingLeft']: 36, [dir === 'rtl' ? 'paddingLeft' : 'paddingRight']: 12, paddingBottom: 8, paddingTop: 8, borderRadius: radius, border: '1.5px solid #E5E7EB', fontSize: 13, fontFamily: 'inherit', outline: 'none', background: '#F9FAFB' }} />
                    </div>
                    ...
                    <Link href={`/store/${slug}/cart`} style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 8, background: primary, color: 'white', borderRadius: radius, padding: '8px 18px', fontWeight: 700, fontSize: 14, textDecoration: 'none' }}>
                        ...
                        {t.storefront.cart}
                        {cartCount > 0 && (
                            <span style={{ position: 'absolute', top: -8, [dir === 'rtl' ? 'left' : 'right']: -8, background: secondary, color: 'white', borderRadius: '50%', width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 900 }}>{cartCount}</span>
                        )}
                    </Link>
                </div>
            </header>
            ...
            {hasSections ? (
                ...
                <main style={{ flex: 1, maxWidth: 1200, margin: '0 auto', padding: '36px 20px', width: '100%', boxSizing: 'border-box' }}>
                    ...
                    <div style={{
                        ...
                        <div>
                            <div style={{ fontSize: 13, fontWeight: 600, opacity: 0.85, marginBottom: 10 }}>{t.storefront.welcomeTo}</div>
                            <h1 style={{ fontSize: 36, fontWeight: 900, lineHeight: 1.3, marginBottom: 20 }}>
                                {store.name_ar || store.name}
                            </h1>
                            {(dir === 'rtl' ? store.description_ar : store.description_en || store.description_ar) && <p style={{ fontSize: 15, opacity: 0.85, marginBottom: 24, maxWidth: 400 }}>{dir === 'rtl' ? store.description_ar : store.description_en || store.description_ar}</p>}
                            <Link href={`/store/${slug}/products`}
                                ...
                                {t.storefront.shopNow} {dir === 'rtl' ? <ChevronLeft size={16} /> : <ChevronRight size={16} />}
                            </Link>
                        </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                        <h2 style={{ fontSize: 20, fontWeight: 900, color: '#111827' }}>{t.storefront.ourProducts}</h2>
                        <span style={{ color: '#6B7280', fontSize: 14 }}>{filteredProducts.length} {t.storefront.productCount}</span>
                    </div>
                    {filteredProducts.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '80px 0', color: '#9CA3AF' }}>
                            <div style={{ fontSize: 48, marginBottom: 16 }}>🔍</div>
                            <p style={{ fontSize: 15 }}>{t.storefront.noProductsFound}</p>
                        </div>
                    ) : (
            ...
            <footer style={{ background: '#111827', color: 'white', marginTop: 80 }}>
                <div style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 48 }}>
                    <div>
                        <div style={{ fontWeight: 900, fontSize: 22, color: primary, marginBottom: 12 }}>{store.name_ar || store.name}</div>
                        <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: 14, lineHeight: 1.8 }}>{dir === 'rtl' ? store.description_ar || t.storefront.reliableStore : store.description_en || store.description_ar || t.storefront.reliableStore}</p>
                    ...
                    <div>
                        <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 16 }}>{t.storefront.quickLinks}</div>
                        {[[t.storefront.home, `/store/${slug}`], [t.storefront.products, `/store/${slug}/products`], [t.storefront.trackOrder, `/store/${slug}/track`]].map(([l, h]) => (
                    ...
                    <div>
                        <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 16 }}>{t.storefront.information}</div>
                        {[t.storefront.privacyPolicy, t.storefront.termsOfUse, t.storefront.returnPolicy].map(l => (
