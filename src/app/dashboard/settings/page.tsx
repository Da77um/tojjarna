...
const TABS = [
    { key: 'general', label: 'المتجر', icon: Store },
...
export default function SettingsPage() {
    ...
    return (
        <div className="page-container" dir="rtl" style={{ paddingBottom: 100 }}>
                            ...
                            return (
                                <button
                                    ...
                                    style={{
                                        ...
                                        border: 'none', textAlign: 'right', fontSize: 14,
                ...
                <div className="settings-page-content">
                    ...
                    {tab === 'general' && (
                        <div className="card card-body" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                            <h3 style={{ fontWeight: 700, fontSize: 16 }}>معلومات المتجر الأساسية</h3>
                            <div className="form-group" style={{ marginBottom: 0 }}>
                                <label className="form-label">اسم المتجر *</label>
                                <input className="form-control" value={storeName} onChange={e => setStoreName(e.target.value)} placeholder="اسم متجرك" />
                            </div>
                            <div className="form-group" style={{ marginBottom: 0 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', border: '1.5px solid #E0D6C8', borderRadius: 10, overflow: 'hidden' }}>
                                    <span style={{ padding: '0 10px', color: '#A09080', fontSize: 12, background: '#F5F0E8', height: 46, display: 'flex', alignItems: 'center', borderLeft: '1px solid #E0D6C8', whiteSpace: 'nowrap', flexShrink: 0 }}>
                                        /store/
                                    </span>
                                    <input style={{ flex: 1, border: 'none', outline: 'none', padding: '0 12px', fontSize: 14, background: 'transparent', fontFamily: 'inherit', direction: 'ltr', minWidth: 0 }}
                    ...
                    {tab === 'appearance' && (
                        <div className="card card-body" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                            <h3 style={{ fontWeight: 700, fontSize: 16 }}>تخصيص مظهر المتجر</h3>
                            <div className="form-group" style={{ marginBottom: 0 }}>
                            ...
                            <div style={{ background: '#F5F0E8', borderRadius: 12, padding: 16 }}>
                                <div style={{ fontSize: 12, fontWeight: 700, color: '#6B6058', marginBottom: 12 }}>معاينة</div>
                                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                                    <button style={{ background: primaryColor, color: 'white', border: 'none', borderRadius: 8, padding: '8px 18px', fontWeight: 700, fontSize: 13, cursor: 'default', fontFamily: 'inherit' }}>أضف إلى السلة</button>
                    ...
                    {tab === 'social' && (
                        <div className="card card-body" style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                            <h3 style={{ fontWeight: 700, fontSize: 16 }}>روابط التواصل الاجتماعي</h3>
                            {[
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
                                <div>
                                    ...
                                    <p style={{ color: '#6B6058', fontSize: 13, marginTop: 4 }}>حدد أسعار التوصيل لكل منطقة</p>
                                </div>
                            ...
                            {shippingZones.length === 0 && (
                                <div className="card card-body" style={{ textAlign: 'center', padding: 40 }}>
                                    ...
                                    <p style={{ color: '#6B6058', fontSize: 14, marginBottom: 12 }}>لم تضف أي مناطق شحن بعد</p>
                                    <button onClick={addZone} className="btn btn-primary btn-sm" style={{ display: 'inline-flex' }}><Plus size={14} />إضافة أولى منطقة</button>
                                </div>
                            )}
                            ...
                            {shippingZones.map(zone => (
                                <div key={zone.id} className="card card-body" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0, paddingBottom: 2 }}>
                                            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#6B6058', cursor: 'pointer' }}>
                                                مفعّلة
                                                <ToggleSwitch value={zone.is_active} onChange={v => updateZone(zone.id, 'is_active', v)} />
                                            </label>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 12 }}>
                                        ...
                                        <div className="form-group" style={{ marginBottom: 0 }}>
                                            <label className="form-label">مجاني فوق (د.أ)</label>
                                            <input className="form-control" type="number" value={zone.free_above || ''} placeholder="غير محدد" onChange={e => updateZone(zone.id, 'free_above', e.target.value ? parseFloat(e.target.value) : null)} />
                                        </div>
                                        <div className="form-group" style={{ marginBottom: 0 }}>
                    ...
                    {tab === 'payments' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                            ...
                            {[
                                { configKey: 'cod_enabled', emoji: '💵', bg: '#D1FAE5', title: 'الدفع عند الاستلام (COD)', sub: 'يدفع العميل نقداً عند وصول الطلب', expandKey: null },
                                { configKey: 'hyperpay_enabled', emoji: '💳', bg: '#EDE9FE', title: 'HyperPay', sub: 'فيزا، ماستركارد، مدى، Apple Pay', expandKey: 'hyperpay_entity_id', expandLabel: 'HyperPay Entity ID' },
                                { configKey: 'paytabs_enabled', emoji: '🔵', bg: '#DBEAFE', title: 'PayTabs', sub: 'بوابة دفع مخصصة للمنطقة العربية', expandKey: 'paytabs_profile_id', expandLabel: 'PayTabs Profile ID' },
                            ].map(p => (
                                <div key={p.configKey} className="card card-body">
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                                        ...
                                        <ToggleSwitch value={payConfig[p.configKey]} onChange={v => setPayConfig((prev: any) => ({ ...prev, [p.configKey]: v }))} />
                                    </div>
                                    {p.expandKey && payConfig[p.configKey] && (
                                        <div className="form-group" style={{ marginBottom: 0, marginTop: 16 }}>
                                            ...
                                            <input className="form-control" value={payConfig[p.expandKey] || ''} onChange={e => setPayConfig((prev: any) => ({ ...prev, [p.expandKey!]: e.target.value }))} placeholder={`أدخل ${p.expandLabel}`} dir="ltr" />
                                        </div>
                                    )}
                                </div>
                            ))}
                    ...
                    {tab === 'domain' && (
                        <div className="card card-body">
                            ...
                            <p style={{ color: '#6B6058', fontSize: 14, marginBottom: 20 }}>اربط نطاقك الخاص بمتجرك لمظهر احترافي</p>
                            <div className="form-group" style={{ marginBottom: 0 }}>
                                ...
                                <div style={{ marginTop: 20, background: '#F5F0E8', borderRadius: 12, padding: 20 }}>
                                    ...
                                    {[
                                        ...
                                        <div key={step} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 12 }}>
                                            ...
                                            <p style={{ fontSize: 13, color: '#6B6058', lineHeight: 1.6, direction: 'ltr', margin: 0 }}>{text}</p>
                                        </div>
                                    ))}
                                </div>
                            )}
